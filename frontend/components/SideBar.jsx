        ...
        <div className="w-52 z-10  md:w-72  bg-[#2f3a4f] h-screen p-4 overflow-y-auto shadow-lg text-gray-50">
            ...
            {course.modules?.map((module, index) => (
                <div key={index} className="mb-2">
                    ...
                    >
                        <span className="text-small  truncate">{module.title.substr(7)}</span>
                        ...
                        <ul className="text-left sm:mt-2">
                            {module.chapters?.map((chapter, chapIndex) => (
                                <li
                                    ...
                                    className={` py-1 sm:py-2 px-1  text-[10px] md:text-sm text-[#c9cbd4] cursor-pointer truncate rounded mb-2  hover:bg-[#5f697b] ${chapter._id==selectedChapter._id? 'border-b':'border-0'  } `}
                                    onClick={() => onSelectChapter(chapter, chapIndex)}
                                >
                                    <span className='mx-1 '></span>{chapter.title}
